...
class Registry extends DatabaseConnection {
    ...
    $loan,$netpay,$gross,$osource,$specify){
        ...
        bank_name, atm_card, loan, monthly_gross, monthly_salary, other_source, specify, max_loanable_amt, date_encoded) 
        VALUES
        ('$code', '$employer','$stype','$tob','$caddress','$alocation','$supname','$hrname','$dhired','$estatus','$msalary',
        '$bname','$atmcard','$esc_loan','$netpay','$gross','$osource','$specify', 0, CURRENT_DATE())";
        return mysqli_query($this->connect(), $sql);
    }
    ...
    function getClientData($code, $table, $codeCol ,$colName){
       ...
       $loan,$netpay,$gross,$osource,$specify){
        $esc_loan = $this->connect()->real_escape_string($loan);
           ...
           bank_name = '$bname', atm_card = '$atmcard', loan = '$esc_loan', monthly_gross = '$netpay', 
           monthly_salary = '$gross', other_source = '$osource', specify = '$specify' WHERE applicant_code = '$code'";
           return mysqli_query($this->connect(), $sql);
       }
